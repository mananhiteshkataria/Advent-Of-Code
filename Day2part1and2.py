...
def parse_game(game):
    ...
    rounds = [round.split('; ') for round in rounds][0]
    rounds = [[(color, int(num)) for num, color in (pair.split(' ') for pair in round.split(', '))] for round in rounds]
...
def is_game_possible(game, red, green, blue):
    game_id, rounds = game
    max_red = max([num for round in rounds for color, num in round if color == 'red'], default=0)
    max_green = max([num for round in rounds for color, num in round if color == 'green'], default=0)
    max_blue = max([num for round in rounds for color, num in round if color == 'blue'], default=0)
    return max_red <= red and max_green <= green and max_blue <= blue

possible_games = [game for game in games if is_game_possible(game, 12, 13, 14)]
sum_of_ids = sum(game_id for game_id, _ in possible_games)

print(sum_of_ids)

def min_cubes_for_game(game):
    game_id, rounds = game
    min_red = max([num for round in rounds for color, num in round if color == 'red'], default=0)
    min_green = max([num for round in rounds for color, num in round if color == 'green'], default=0)
    min_blue = max([num for round in rounds for color, num in round if color == 'blue'], default=0)
    return min_red, min_green, min_blue

min_cubes = [min_cubes_for_game(game) for game in games]

power_of_sets = [red * green * blue for red, green, blue in min_cubes]
